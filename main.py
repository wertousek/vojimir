...
async def on_message(message):
	...
	await spamProtection(message, choice([f"{message.author.mention} nespamuj!",f"{message.author.mention} ještě jednou něco napíšeš a dostaneš do držky.",f"{message.author.mention}už nic nepiš! bolí mě z toho hlava!"]), 5)
	...
	if "prodej" == commandos:
		if attributes == None:
			...
			await message.channel.send(f"Hráče prodej za {int(int(attributes)*0.8)}£, {int(int(attributes)*0.85)}£ až {int(int(attributes)*0.9)}£")
